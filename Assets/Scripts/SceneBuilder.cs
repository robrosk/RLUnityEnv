...
public class SceneBuilder : MonoBehaviour
{
    ...
    private void Update()
    {
        // Editor-only: Check for the "Generate Now" button press
        if (createNow)
        {
            createNow = false;
            CreateBasicScene();
    ...
    private GameObject CreateAgentPrefab()
    {
        ...
        // Simplified setup for compatibility with more ML-Agents versions
        #if UNITY_2020_1_OR_NEWER
            // For newer ML-Agents (1.0+)
            behaviorParams.BrainParameters.VectorObservationSize = 9;
            behaviorParams.BrainParameters.NumStackedVectorObservations = 1;
            behaviorParams.BrainParameters.ActionSpec = ActionSpec.MakeContinuous(3);
        #else
            // For older ML-Agents
            behaviorParams.BrainParameters.vectorObservationSize = 9;
            behaviorParams.BrainParameters.numStackedVectorObservations = 1;
            behaviorParams.BrainParameters.vectorActionSize = new int[] { 3 };
            behaviorParams.BrainParameters.vectorActionSpaceType = SpaceType.Continuous;
        #endif
    ...
    private void CreateLimb(string name, Transform parent, Vector3 position, Vector3 scale, Material material)
    {
